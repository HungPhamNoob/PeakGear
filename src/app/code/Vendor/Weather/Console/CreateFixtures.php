        ...
        foreach ($this->categories as $parentName => $children) {
            ...
            try {
                ...
                $parentCat->setName($parentName)
                          ...
                          ->setData('description', 'Danh muc ' . $parentName . ' - PeakGear outdoor shop')
                          ->setData('url_key', $this->makeUrlKey($parentName) . '-pg' . $parentCount);
                $parentCat->save();

                $output->writeln("✅ Tao danh muc cha #{$parentCat->getId()}: {$parentName}");
                ...
                $output->writeln("<error>Loi danh muc {$parentName}: {$e->getMessage()}</error>");
            }
        }

        $output->writeln("\n<info>=== Tao san pham demo ===</info>");
        $productCount = 0;
