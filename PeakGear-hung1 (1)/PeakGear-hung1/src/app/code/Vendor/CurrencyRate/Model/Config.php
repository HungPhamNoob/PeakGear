...
class Config
{
    ...
    {
        $configured = trim((string)$this->scopeConfig->getValue(self::XML_PATH_CURRENCIES, ScopeInterface::SCOPE_STORE));
        if ($configured === '' || $configured === '*' || strtoupper($configured) === 'ALL') {
            return [];
        }
